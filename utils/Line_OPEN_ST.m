  ...
  while (n>1)
       ...
       y_n2 = parentPP_node(1,2) - parentP_node(1,2) ;
        ...
        if angle==0         %%%夹角为0，说明在一条直线上，修改父节点
            ...
            % 防止除零：当两点x坐标相同时，使用垂直线判断
            if abs(x3 - x1) < 0.001
                % 垂直线情况
                for j=1:1:Num_obs
                    x_obn=CLOSED(j,1); y_obn=CLOSED(j,2);
                    if (x_obn>=x_min-0.8 && x_obn<=x_max+0.8 && y_obn>=y_min && y_obn<=y_max)
                        d = abs(x_obn - x1);
                        if d <= 1
                            f=1;
                        end
...
%                   if y_obn>=(yline-1.2)&&y_obn<=(yline+1.2)
%                       f=1;
%                   end
%                   xline=(y_obn-b)/k;
%                   if x_obn>=(xline-1.2)&&x_obn<=(xline+1.2)
%                       f=1;
%                   end
